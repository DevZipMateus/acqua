...
const HeroSection = () => {
  ...
  return <section className={`relative min-h-screen flex items-center justify-center overflow-hidden acqua-gradient ${spacing.section}`} ref={sectionRef}>
      {/* Background Video/Image */}
      <div className="absolute inset-0 z-0">
        <img src="/lovable-uploads/9b77b80c-077b-4126-9e45-17e877157862.png" alt="Warehouse with organized materials" className="w-full h-full object-cover opacity-20" />
        <div className="absolute inset-0 bg-gradient-to-b from-acqua-900/50 to-acqua-600/30 bg-[#000a00]/[0.49]"></div>
      </div>
      ...
      <div className={`relative z-10 ${containerClass} mx-auto text-center`}>
        <div className={`${isUltraWide ? 'max-w-6xl' : isMobile ? 'max-w-sm' : 'max-w-4xl'} mx-auto px-2`}>
          ...
          <h1 className={`font-bold text-white mb-4 leading-tight opacity-0 animate-fadeIn ${textSizes.title}`}>
            Soluções em <span className="text-gradient text-[#22ce22]/[0.56]">Vedações</span>
          </h1>
          ...
          <div className={`flex flex-col space-y-3 justify-center items-center opacity-0 animate-fadeIn mb-6`}>
            <Button onClick={scrollToContact} size={isMobile ? "sm" : "lg"} className={`bg-acqua-600 hover:bg-acqua-700 text-white font-semibold rounded-full ${isMobile ? 'px-4 py-2 text-sm w-full max-w-xs' : 'px-8 py-4 text-lg'}`}>
              Solicitar Orçamento
              <ArrowRight className={`ml-2 ${isMobile ? 'w-3 h-3' : 'w-5 h-5'}`} />
            </Button>
            
            <Button onClick={scrollToServices} variant="outline" size={isMobile ? "sm" : "lg"} className={`border-2 border-white text-white hover:bg-white hover:text-acqua-900 font-semibold rounded-full ${isMobile ? 'px-4 py-2 text-sm w-full max-w-xs' : 'px-8 py-4 text-lg'}`}>
          ...
          <div className="opacity-0 animate-fadeIn">
            ...
            <div className={`flex flex-col space-y-2 justify-center items-center text-white ${isMobile ? 'text-xs' : 'text-base'}`}>
              <a href="tel:+5519999300066" className="flex items-center hover:text-acqua-300 transition-colors">
                📞 (19) 99930-0066
              </a>
              <a href="mailto:comercial@acquaambiental.com.br" className="flex items-center hover:text-acqua-300 transition-colors">
      ...
      {/* Scroll Indicator */}
      {!isMobile && <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 opacity-0 animate-fadeIn">
